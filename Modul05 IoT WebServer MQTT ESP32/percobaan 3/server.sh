#!/usr/bin/env bash
# server.sh – Pengelola server Percobaan 3 (SSE Sensor Dashboard)
set -e
...
print_menu() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║    Percobaan 3 – Server Manager      ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════╝${NC}"
  echo ""
  echo "  1) Start Backend  (FastAPI :8003)"
  echo "  2) Start Frontend (Vite dev :9003)"
...
start_backend() {
  echo -e "${GREEN}[Backend] Menjalankan FastAPI pada port 8003...${NC}"
  cd "$BACKEND_DIR"
  if [ ! -f .env ]; then
    cp .env.example .env
...
start_frontend() {
  echo -e "${GREEN}[Frontend] Menjalankan Vite dev server pada port 9003...${NC}"
  cd "$FRONTEND_DIR"
  npm run dev
}
...
build_frontend() {
  echo -e "${GREEN}[Frontend] Membangun untuk produksi...${NC}"
  cd "$FRONTEND_DIR"
  npm run build
...
full_setup() {
  ...
  sleep 2
  echo -e "${BLUE}[Setup] Memulai frontend...${NC}"
  cd "$FRONTEND_DIR"
  npm run dev
}
