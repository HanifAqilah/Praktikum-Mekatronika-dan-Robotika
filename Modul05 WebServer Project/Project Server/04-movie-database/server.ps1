...
function Show-Menu {
    Write-Host ""
    Write-Host "===================================================="
    Write-Host " Project 4: Movie Database Search"
    Write-Host " Stack: React + Axios | TMDB API + localStorage"
    Write-Host "===================================================="
    ...
    Write-Host "9. Test TMDB API Key"
...
function Install-Deps {
    if (Test-Path -LiteralPath (Join-Path $ProjectDir "package.json")) {
        ...
        Write-Host "TMDB key docs: https://www.themoviedb.org/documentation/api"
    }
}
...
function Start-Server {
    ...
    if ((-not (Test-Path -LiteralPath (Join-Path $ProjectDir ".env"))) -and (-not (Test-Path -LiteralPath (Join-Path $ProjectDir ".env.local")))) {
        Write-Host "No .env/.env.local found. You may need VITE_TMDB_API_KEY."
    }
...
function Show-Status {
    Write-Host ""
    Write-Host "Server Status - Movie Database Search"
    Write-Host "--------------------------------------"
    $proc = Get-SavedProcess
    if ($proc) {
    ...
    foreach ($file in @((Join-Path $ProjectDir ".env"), (Join-Path $ProjectDir ".env.local"))) {
        if ((Test-Path -LiteralPath $file) -and ((Get-Content -LiteralPath $file -Raw) -match "TMDB|MOVIE")) {
            $found = $true
            break
        }
    }
    if ($found) {
        Write-Host "TMDB API Key : Found in .env/.env.local"
    } else {
        Write-Host "TMDB API Key : Not found. Set VITE_TMDB_API_KEY"
    }
    Write-Host ""
}
...
function Get-TmdbApiKey {
    $patterns = @(
        '^\s*VITE_TMDB_API_KEY\s*=\s*(.+)$',
        '^\s*REACT_APP_TMDB_API_KEY\s*=\s*(.+)$',
        '^\s*TMDB_API_KEY\s*=\s*(.+)$'
    )
    foreach ($file in @((Join-Path $ProjectDir ".env"), (Join-Path $ProjectDir ".env.local"))) {
...
function Test-TmdbApi {
    ...
    if ([string]::IsNullOrWhiteSpace($apiKey)) {
        Write-Host "No TMDB API key found in .env/.env.local"
        return
    }
    $uri = "https://api.themoviedb.org/3/movie/popular?api_key=$apiKey&page=1"
    try {
        $res = Invoke-RestMethod -Uri $uri -Method Get -TimeoutSec 20
        if ($null -ne $res.results) {
            Write-Host "TMDB API key is valid"
            Write-Host "Popular movies returned: $($res.results.Count)"
        } else {
            Write-Host "TMDB API key test failed"
        }
    } catch {
        Write-Host "TMDB API key test failed: $($_.Exception.Message)"
    }
}
